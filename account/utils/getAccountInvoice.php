    ...
    foreach ($show1 as $arrData1) {
        ...
        foreach ($arrData1 as $row1) {
            if ($row1['status'] == "Invoiced") {
                ...
                <td>' . $driverName1 . '</td>
                <td>
                    <select class="form-control" onchange="updateLoadStatus1(' . $activeLoadId1 . ')" id="loadStatus1"
                            style="width: 170px">
...
if ($_GET['type'] == 'search_text1') {
    ...
    foreach ($show1 as $row) {
        ...
        foreach ($trailer as $row4) {
            ...
            $status1 = $row4['status'];
            foreach ($row4['shipper'] as $shipperData1) {
                $shipDate1 = $shipperData1['shipper_pickup'];
            }
            if ($_POST['getoption'] == $row4['_id']
                ...
                <td>' . $driverName1 . '</td>
                <td>
                    <select class="form-control" onchange="updateLoadStatus1(' . $invoiceID1 . ')" id="loadStatus1"
                            style="width: 170px">
                        <option value="0">--Select--</option>';
