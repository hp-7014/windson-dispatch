...
!function (a) {
    ...
    function e() {
        if (d()) {
            var a = parseFloat(("" + (/CPU.*OS ([0-9_]{1,5})|(CPU like).*AppleWebKit.*Mobile/i.exec(navigator.userAgent) || [0, ""])[1]).replace("undefined", "3_2").replace("_", ".").replace("_", ""));
            return a
        }
        return 0
    }
    ...
    f.DEFAULTS = {
        ...
        var b = this;
        this.$btnToolbar = a('[data-responsive-table-toolbar="' + this.id + '"]').addClass("btn-toolbar"), 0 === this.$btnToolbar.length && (this.$btnToolbar = a('<div class="btn-toolbar" />')), this.$dropdownGroup = a('<div class="btn-group dropdown-btn-group"style="margin-left: 5px;"/>'), this.$dropdownBtn = a('<button type="button" class="btn btn-secondary dropdown-toggle" data-toggle="dropdown">' + this.options.i18n.display + ' <span class="caret"></span></button>'), this.$dropdownContainer = a('<ul class="dropdown-menu"/>'), this.options.addFocusBtn && (this.$focusGroup = a('<div class="btn-group focus-btn-group" />'), this.$focusBtn = a('<button type="button" class="btn btn-secondary" style="margin-left: 5px;">' + this.options.i18n.focus + "</button>"), this.options.focusBtnIcon && this.$focusBtn.prepend('<span class="' + this.options.focusBtnIcon + '"></span> '), this.$focusGroup.append(this.$focusBtn), this.$btnToolbar.append(this.$focusGroup), this.$focusBtn.click(function () {
            a.proxy(b.activateFocus(), b)
        }), this.$bodyRows.click(function () {
            a.proxy(b.focusOnRow(a(this)), b)
        })), this.options.addDisplayAllBtn && (this.$displayAllBtn = a('<button type="button" class="btn btn-secondary">' + this.options.i18n.displayAll + "</button>"), this.$dropdownGroup.append(this.$displayAllBtn), this.$table.hasClass("display-all") && this.$displayAllBtn.addClass("btn-primary"), this.$displayAllBtn.click(function () {
        ...
        }), b.$tableClone.wrap('<div class="sticky-table-header"/>'), b.$stickyTableHeader = b.$tableClone.parent(), b.$stickyTableHeader.css("height", b.$thead.height() + 2), b.$table.before(b.$stickyTableHeader), a(this.options.mainContainer).bind("scroll", function () {
            a.proxy(b.updateStickyTableHeader(), b)
        }), a(window).bind("resize", function (c) {
            a.proxy(b.updateStickyTableHeader(), b)
        }), a(b.$tableScrollWrapper).bind("scroll", function () {
            a.proxy(b.updateStickyTableHeader(), b)
        }), b.useFixedSolution = !d() || e() >= 8, b.useFixedSolution ? b.$tableScrollWrapper.addClass("fixed-solution") : b.$tableScrollWrapper.addClass("absolute-solution")
    }, f.prototype.updateStickyTableHeader = function () {
        var b = this, c = 0, d = b.$table.offset().top, e = a(this.options.mainContainer).scrollTop() - 1,
            f = b.$table.height() - b.$stickyTableHeader.height(),
            g = e + a(this.options.mainContainer).height() - a(document).height(), h = 0;
        if (a(b.options.fixedNavbar).length) {
            var i = a(b.options.fixedNavbar).first();
            h = i.height(), e += h
        }
        var j;
        if (j = this.options.mainContainer === window ? e > d && e < d + b.$table.height() : d <= 0 && -d < b.$table.height(), b.useFixedSolution) {
            if (b.$stickyTableHeader.scrollLeft(b.$tableScrollWrapper.scrollLeft()), c = h - 1, this.options.mainContainer === window && e - d > f ? (c -= e - d - f, b.$stickyTableHeader.addClass("border-radius-fix")) : this.options.mainContainer !== window && -d > f ? (c -= -d - f, b.$stickyTableHeader.addClass("border-radius-fix")) : b.$stickyTableHeader.removeClass("border-radius-fix"), j) return void b.$stickyTableHeader.css({
                visibility: "visible",
                top: c + "px",
            ...
            var k = 400;
            c = this.options.mainContainer === window ? e - d - 1 : -d - 1, c < 0 ? c = 0 : c > f && (c = f), this.options.mainContainer === window && g > 0 && (c -= g), j ? (b.$stickyTableHeader.css({visibility: "visible"}), b.$stickyTableHeader.animate({top: c + "px"}, k), b.$thead.css({visibility: "hidden"})) : b.$stickyTableHeader.animate({top: "0"}, k, function () {
        ...
        b.$hdrCells.each(function (c) {
            ...
            if (e || (e = b.idPrefix + c, d.prop("id", e)), "" === f && (f = d.attr("data-col-name")), d.is("[data-priority]") && d.data("priority") !== -1) {
                ...
                b.$dropdownContainer.append(g), g.click(function () {
                    ...
                    b.$table.hasClass("display-all") && (a.proxy(b.preserveDisplayAll(), b), b.$table.removeClass("display-all"), b.$tableClone && b.$tableClone.removeClass("display-all"), b.$displayAllBtn.removeClass("btn-primary")), e.each(function () {
                        var b = a(this);
                        if (c.is(":checked")) {
                            if (!b.closest("thead").length && "none" !== b.css("display")) {
                                var d = Math.min(parseInt(b.prop("colSpan")) + 1, b.attr("data-org-colspan"));
                                b.prop("colSpan", d)
                            }
                            b.show()
        ...
        }), a.isEmptyObject(this.headerRowIndices) || b.setupRow(this.$thead.find("tr:eq(1)"), this.headerRowIndices)
        ...
        if (!b.data("setup")) {
            ...
            b.find("th, td").each(function () {
                var b = a(this), f = "", g = b.prop("colSpan");
                b.attr("data-org-colspan", g), g > 1 && b.addClass("spn-cell");
                for (var h = e; h < e + g; h++) {
                    f = f + " " + d.idPrefix + c[h];
                    var i = d.$table.find("#" + d.idPrefix + c[h]), j = i.attr("data-priority");
        ...
        var d = 0, e = 0;
        if (this.$thead.find("tr").first().find("th").each(function (f) {
            for (var g = a(this), h = g.prop("colSpan"), i = g.prop("rowSpan"), j = 0; j < h; j++) b.headerColIndices[d + f + j] = f, d + f + j >= 0 && (c[d + f + j - e] = e);
            i > 1 && e++, d += h - 1
        }), this.$thead.find("tr").length > 2) throw new Error("This plugin doesnt support more than two rows in thead.");
        ...
        b.find("[id]").each(function () {
            a(this).prop("id", a(this).prop("id") + "-clone")
        }), b.appendTo(this.$tableClone), this.$dropdownContainer.find("input").trigger("change")
    ...
    a.fn.responsiveTable = function (b) {
        return this.each(function () {
            var c = a(this), d = c.data("responsiveTable"),
                e = a.extend({}, f.DEFAULTS, c.data(), "object" == typeof b && b);
            "" !== e.pattern && (d || c.data("responsiveTable", d = new f(this, e)), "string" == typeof b && d[b]())
        })
    }, a.fn.responsiveTable.Constructor = f, a.fn.responsiveTable.noConflict = function () {
